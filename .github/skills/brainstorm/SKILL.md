---
name: brainstorm
description: "Guided game concept ideation 鈥?from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration."
...
3. **Run through ideation phases** interactively, asking the user questions at
   each phase. Do NOT generate everything silently 鈥?the goal is **collaborative
   ...
   **Use `AskUserQuestion`** at key decision points throughout brainstorming:
   ...
   - Withhold judgment 鈥?no idea is bad during exploration
   - Encourage unusual ideas 鈥?outside-the-box thinking sparks better concepts
   - Build on each other 鈥?"yes, and..." responses, not "but..."
   - Use constraints as creative fuel 鈥?limitations often produce the best ideas
   - Time-box each phase 鈥?keep momentum, don't over-deliberate early
...
**Synthesize** the answers into a **Creative Brief** 鈥?a 3-5 sentence
...
- **Working Title**
- **Elevator Pitch** (1-2 sentences 鈥?must pass the "10-second test")
...
new concepts. Never pressure toward a choice 鈥?let them sit with it.
...
The core loop is the beating heart of the game 鈥?if it isn't fun in
...
- Is this action intrinsically satisfying? (Would they do it with no
  rewards, no progression, no story 鈥?just for the feel of it?)
...
- Pillars should feel like they create tension with each other 鈥?if all
...
4. **Generate the game concept document** using the template at
   `.copilot/docs/templates/game-concept.md`. Fill in ALL sections from the
...
6. **Suggest next steps** (in this order 鈥?this is the professional studio
   ...
   - "Decompose the concept into individual systems with `/map-systems` 鈥?maps dependencies, assigns priorities, and creates the systems index"
   - "Author per-system GDDs with `/design-system` 鈥?guided, section-by-section GDD writing"
