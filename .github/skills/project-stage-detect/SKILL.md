...
Based on scanned artifacts, determine stage. Check `production/stage.txt` first 鈥?if it exists, use its value (explicit override from `/gate-check`). Otherwise,
...
| **Polish** | Explicit only (set by `/gate-check` Production 鈫?Polish gate) |
| **Release** | Explicit only (set by `/gate-check` Polish 鈫?Release gate) |
...
Use template: `.copilot/docs/templates/project-stage-report.md`
...
- **Concept exists but no systems index?** 鈫?`/map-systems` to decompose into systems
- **Missing design docs?** 鈫?`/reverse-document design src/[system]`
- **Missing architecture docs?** 鈫?`/architecture-decision` or `/reverse-document architecture`
- **Prototypes need documentation?** 鈫?`/reverse-document concept prototypes/[name]`
- **No sprint plan?** 鈫?`/sprint-plan`
- **Approaching milestone?** 鈫?`/milestone-review`
