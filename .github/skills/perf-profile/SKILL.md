...
2. **Read performance budgets** 鈥?Check for existing performance targets in design docs or COPILOT.md:
...
3. **Analyze the codebase** for common performance issues:
   ...
   **CPU Profiling Targets**:
   - `_process()` / `Update()` / `Tick()` functions 鈥?list all and estimate cost
...
4. **Generate the profiling report**:
   ...
   1. **[Title]** 鈥?[Description of the optimization]
      - Location: [file:line]
...
### Rules
- Never optimize without measuring first 鈥?gut feelings about performance are unreliable
- Recommendations must include estimated impact 鈥?"make it faster" is not actionable
