#!/bin/bash
# GitHub Copilot PreCompact hook: Dump session state before context compression
...
if [ -f "$STATE_FILE" ]; then
    ...
    if [ "$STATE_LINES" -gt 100 ] 2>/dev/null; then
        ...
        echo "... (truncated 鈥?$STATE_LINES total lines, showing first 100)"
    else
        cat "$STATE_FILE"
    fi
else
...
echo ""
echo "## Design Docs 鈥?Work In Progress"
