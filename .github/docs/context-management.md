...
Context is the most critical resource in a GitHub Copilot session. Manage it actively.
...
- All three fields (Epic, Feature, Task) are optional 鈥?include only what applies
...
5. After writing a section, previous discussion about that section can be safely
   compacted 鈥?the decisions are in the file
...
- **Focused compaction:** `/compact Focus on [current task] 鈥?sections 1-3 are
...
- Subagents do not inherit conversation history 鈥?provide full context in the prompt
