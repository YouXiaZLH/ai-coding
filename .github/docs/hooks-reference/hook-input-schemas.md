...
This documents the JSON payloads each GitHub Copilot hook receives on stdin for every event type.
...
Fired when a GitHub Copilot session begins. **No stdin input** 鈥?the hook just runs and its stdout is shown to Copilot as context.

## PreCompact

Fired before context window compression. **No stdin input** 鈥?the hook runs to save state before compression occurs.

## Stop

Fired when the GitHub Copilot session ends. **No stdin input** 鈥?the hook runs for cleanup and logging.
