...
- Clarify before assuming 鈥?specs are never 100% complete
- Propose architecture, don't just implement 鈥?show your thinking
- Explain trade-offs transparently 鈥?there are always multiple valid approaches
- Flag deviations from design docs explicitly 鈥?designer should know if implementation differs
- Rules are your friend 鈥?when they flag issues, they're usually right
- Tests prove it works 鈥?offer to write them proactively
...
- Prefer composition over inheritance 鈥?attach behavior via child nodes, not deep class hierarchies
- Each scene should be self-contained and reusable 鈥?avoid implicit dependencies on parent nodes
...
- Keep the scene tree shallow 鈥?deep nesting causes performance and readability issues
...
- Signals for decoupled communication 鈥?prefer signals over direct method calls between nodes
- Use `await` for async operations (signals, timers, tweens) 鈥?never use `yield` (Godot 3 pattern)
...
- Connect signals in `_ready()` or via the editor 鈥?never in `_process()`
- Use signal bus (autoload) for global events, direct signals for parent-child
- Avoid connecting the same signal multiple times 鈥?check `is_connected()` or use `connect(CONNECT_ONE_SHOT)`
- Type-safe signal parameters 鈥?always include types in signal declarations

### Performance
- Minimize `_process()` and `_physics_process()` 鈥?disable with `set_process(false)` when idle
...
- Profile with Godot's built-in profiler and monitors 鈥?check `Performance` singleton

### Autoloads
- Use sparingly 鈥?only for truly global systems (audio manager, save system, events bus)
- Autoloads must not depend on scene-specific state
- Never use autoloads as a dumping ground for convenience functions
- Document every autoload's purpose in COPILOT.md
...
- Not freeing nodes (`queue_free()`) 鈥?watch for memory leaks with orphan nodes
...
- `subagent_type: godot-gdscript-specialist` 鈥?GDScript architecture, static typing, signals, coroutines
- `subagent_type: godot-shader-specialist` 鈥?Godot shading language, visual shaders, particles
- `subagent_type: godot-gdextension-specialist` 鈥?C++/Rust bindings, native performance, custom nodes
