# GitHub Copilot Game Studios -- Complete Workflow Guide

> **How to go from zero to a shipped game using the Agent Architecture.**
>
> This guide walks you through every phase of game development using the
> 48-agent system, 37 slash commands, and automated hooks. It assumes you
> have GitHub Copilot installed and are working from the project root.
...
Run `/setup-engine` in GitHub Copilot. This is the single most important
...
- Pins the engine and version in `COPILOT.md`
- Populates `.copilot/docs/technical-preferences.md` with naming conventions,
...
> `COPILOT.md` directly if you prefer.

### Step 0.3: Verify Hooks Are Working

Start a new GitHub Copilot session. You should see output from the
`session-start.sh` hook:

```
=== GitHub Copilot Game Studios -- Session Context ===
Branch: main
Recent commits:
...
If you see this, hooks are working. If not, check `.copilot/settings.json` to
make sure the hook paths are correct for your OS.

### Step 0.4: Create Your Directory Structure

The directories listed in `COPILOT.md` don't all exist yet. Create them as
...
.copilot/docs/templates/game-concept.md
```
...
.copilot/docs/templates/game-pillars.md
```
...
This creates `design/gdd/systems-index.md` 鈥?a master tracking document that:
...
- Determines design order (Foundation 鈫?Core 鈫?Feature 鈫?Presentation 鈫?Polish)
...
.copilot/docs/templates/game-design-document.md
```

**The `design-docs` rule** (`.copilot/rules/design-docs.md`) requires every GDD
...
The `narrative` rule (`.copilot/rules/narrative.md`) enforces:
...
.copilot/docs/templates/milestone-definition.md
```
...
**Key rule:** The `prototype-code` rule (`.copilot/rules/prototype-code.md`)
...
```
1. Start a GitHub Copilot session
...
Agent (coordinator): "Feature complete. Status:
       鉁?Design, architecture, implementation, integration done
       鉁?Critical bugs fixed
...
Agent: "Reviewing src/gameplay/combat/damage_calculator.gd...
       ...
       **Architectural Compliance**:
       鉁?Matches ADR-003 (stateless utility pattern)
       鉁?Data-driven values (gameplay-code rule)
       鉁?Issue: Direct reference to HealthComponent - breaks dependency injection

       **SOLID Principles**:
       鉁?Single Responsibility - only calculates damage
       鉁?Dependency Inversion - tightly coupled to HealthComponent type

       **Testability**:
       鉁?Hard to mock HealthComponent dependency

       **Performance**:
       鉁?No allocations in hot path
       鈿?Warning: Type lookup in dictionary every call - consider caching
...
The `test-standards` rule (`.copilot/rules/test-standards.md`) enforces:
...
Agent (coordinator): "Polish pass complete:
       鉁?Performance: 48fps -> 62fps
       鉁?Visual: Enhanced VFX, type-specific effects, crit feedback
       鉁?Feel: Hitstop, particles, camera shake
       鉁?Tests: All passing
       Ready to merge."
...
Agent: "Starting release coordination for v1.0.0. First, status check across
       ...
       鉁?Engineering: All green
       鉁?QA: 2 minor bugs open (non-blocking)
       鉁?Art/Audio: Complete
       鉁?Localization: Complete
       鈿?Store: Screenshots pending approval
       鉁?Marketing: Launch trailer not finalized
...
Agent: "Creating release build v1.0.0...
       - Windows build: 鉁?Complete, tested, 2.1GB
       - Linux build: 鉁?Complete, tested, 2.3GB
       - Checksums generated
...
Agent: "Running final regression suite on release builds...
       - All critical tests: 鉁?Pass
       - All high-priority tests: 鉁?Pass
       - Platform-specific tests: 鉁?Pass
...
Agent: "Final go/no-go check:
       鉁?All builds validated
       鉁?QA sign-off complete
       鉁?Store metadata approved
       鉁?Launch trailer complete (confirmed)
       鉁?Marketing materials ready
       鉁?Support team briefed
       鉁?Deployment plan approved
...
.copilot/docs/templates/post-mortem.md
```
...
1. /start (asks where you are, routes you to the right workflow)
   鈥?or /brainstorm if you prefer to jump straight to ideation
