#!/bin/bash
# GitHub Copilot SessionStart hook: Load project context at session start
# Outputs context information that Copilot sees when a session begins
#
# Input schema (SessionStart): No stdin input

echo "=== GitHub Copilot Game Studios 鈥?Session Context ==="
...
if [ -f "$STATE_FILE" ]; then
    ...
    if [ "$TOTAL_LINES" -gt 20 ]; then
        echo "  ... ($TOTAL_LINES total lines 鈥?read the full file to continue)"
    fi
