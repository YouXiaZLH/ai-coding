...
1. **Concept** 鈥?Brainstorming, game concept document
2. **Systems Design** 鈥?Mapping systems, writing GDDs
3. **Technical Setup** 鈥?Engine config, architecture decisions
4. **Pre-Production** 鈥?Prototyping, vertical slice validation
5. **Production** 鈥?Feature development (Epic/Feature/Task tracking active)
6. **Polish** 鈥?Performance, playtesting, bug fixing
7. **Release** 鈥?Launch prep, certification
...
- **With argument**: `/gate-check production` 鈥?validate readiness for that specific phase
...
### Gate: Systems Design 鈫?Technical Setup
...
### Gate: Technical Setup 鈫?Pre-Production

**Required Artifacts:**
- [ ] Engine chosen (COPILOT.md Technology Stack is not `[CHOOSE]`)
- [ ] Technical preferences configured (`.copilot/docs/technical-preferences.md` populated)
...
### Gate: Pre-Production 鈫?Production
...
- Don't just check existence 鈥?verify the file has real content (not just a template header)
...
```
## Gate Check: [Current Phase] 鈫?[Target Phase]

**Date**: [date]
**Checked by**: gate-check skill

### Required Artifacts: [X/Y present]
- [x] design/gdd/game-concept.md 鈥?exists, 2.4KB
- [ ] docs/architecture/ 鈥?MISSING (no ADRs found)
- [x] production/sprints/ 鈥?exists, 1 sprint plan

### Quality Checks: [X/Y passing]
- [x] GDD has 8/8 required sections
- [ ] Tests 鈥?FAILED (3 failures in tests/unit/)
- [?] Core loop playtested 鈥?MANUAL CHECK NEEDED

### Blockers
1. **No Architecture Decision Records** 鈥?Run `/architecture-decision` to create one
   covering core system architecture before entering production.
2. **3 test failures** 鈥?Fix failing tests in tests/unit/ before advancing.
...
Example: if passing the "Pre-Production 鈫?Production" gate:
```bash
...
- **No game concept?** 鈫?`/brainstorm` to create one
- **No systems index?** 鈫?`/map-systems` to decompose the concept into systems
- **Missing design docs?** 鈫?`/reverse-document` or delegate to `game-designer`
- **Missing ADRs?** 鈫?`/architecture-decision`
- **Tests failing?** 鈫?delegate to `lead-programmer` or `qa-tester`
- **No playtest data?** 鈫?`/playtest-report`
- **Performance unknown?** 鈫?`/perf-profile`
- **Not localized?** 鈫?`/localize`
- **Ready for release?** 鈫?`/launch-checklist`
...
4. **User decides**: The verdict is a recommendation 鈥?the user makes the final call
5. **Get approval**: "May I write this gate check report to production/gate-checks/?"

**Never** block a user from advancing 鈥?the verdict is advisory. Document the risks
