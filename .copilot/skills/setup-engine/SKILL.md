---
name: setup-engine
description: "Configure the project's game engine and version. Pins the engine in COPILOT.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyond the LLM's training data."
...
- **Full spec**: `/setup-engine godot 4.6` 鈥?engine and version provided
- **Engine only**: `/setup-engine unity` 鈥?engine provided, version will be looked up
- **No args**: `/setup-engine` 鈥?fully guided mode (engine recommendation + version)
...
- Read `design/gdd/game-concept.md` if it exists 鈥?extract genre, scope, platform
...
- If no concept exists, inform the user:
  ...
  > you want to build 鈥?it will also recommend an engine. Or tell me about your
...
Let the user choose 鈥?never force a recommendation.
...
After updating COPILOT.md, create or update `.copilot/docs/technical-preferences.md` with
...
- **Within training data** 鈫?`LOW RISK` 鈥?reference docs optional but recommended
- **Near the edge** 鈫?`MEDIUM RISK` 鈥?reference docs recommended
- **Beyond training data** 鈫?`HIGH RISK` 鈥?reference docs required
...
| **LLM Knowledge Cutoff** | May 2025 |
| **Risk Level** | LOW 鈥?version is within LLM training data |
...
Do NOT create breaking-changes.md, deprecated-apis.md, etc. 鈥?they would
...
3. **Create the full reference directory**:
   ...
   鈹溾攢鈹€ VERSION.md              # Version pin + knowledge gap analysis
   鈹溾攢鈹€ breaking-changes.md     # Version-by-version breaking changes
   鈹溾攢鈹€ deprecated-apis.md      # "Don't use X 鈫?Use Y" tables
   鈹溾攢鈹€ current-best-practices.md  # New practices since training cutoff
   鈹斺攢鈹€ modules/                # Per-subsystem references (create as needed)
   ```
...
- NEVER guess an engine version 鈥?always verify via WebSearch or user confirmation
- NEVER overwrite existing reference docs without asking 鈥?append or update
- If reference docs already exist for a different engine, ask before replacing
- Always show the user what you're about to change before making COPILOT.md edits
