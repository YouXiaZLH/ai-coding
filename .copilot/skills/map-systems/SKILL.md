...
- **No argument**: `/map-systems` 鈥?Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: `/map-systems next` 鈥?Pick the highest-priority undesigned system
...
**Required:**
- Read `design/gdd/game-concept.md` 鈥?**fail with a clear message if missing**:
...
- Read `design/gdd/game-pillars.md` 鈥?pillars constrain priority and scope
- Read `design/gdd/systems-index.md` 鈥?if exists, **resume** from where it left off
  (update, don't recreate from scratch)
- Glob `design/gdd/*.md` 鈥?check which system GDDs already exist
...
skill 鈥?it requires human judgment because concept docs rarely enumerate every
system explicitly.
...
- Any "bottleneck" systems (many others depend on them 鈥?these are high-risk)
- Any systems with no dependents (leaf nodes 鈥?lower risk, can be designed late)
...
requires it 鈥?without [system], the 30-second loop can't function."
...
Using the template at `.copilot/docs/templates/systems-index.md`, populate the
