...
> "Usage: `/design-system <system-name>` 鈥?e.g., `/design-system combat-system`
...
primary advantage over ad-hoc design 鈥?it arrives informed.

### 2a: Required Reads

- **Game concept**: Read `design/gdd/game-concept.md` 鈥?fail if missing:
  > "No game concept found. Run `/brainstorm` first."
- **Systems index**: Read `design/gdd/systems-index.md` 鈥?fail if missing:
...
Use the template structure from `.copilot/docs/templates/game-design-document.md`:
...
- Why does this system exist 鈥?what would the game lose without it?
...
**Goal**: The emotional target 鈥?what the player should *feel*.
...
this system, reference it explicitly. Don't reinvent 鈥?connect.
...
reference them here. Don't create duplicate knobs 鈥?point to the source of truth.
...
- "The 8 required sections are complete. Do you want to also define Visual/Audio
  ...
  - Options: "Yes, all three", "Just open questions", "Skip 鈥?I'll add these later"
...
Read back the complete GDD from file (not from conversation memory 鈥?the file is
...
- Update the target system's row:
  - If design-review was run and verdict is APPROVED: Status 鈫?"Approved"
  - If design-review was run and verdict is NEEDS REVISION: Status 鈫?"In Review"
  - If design-review was skipped: Status 鈫?"Designed" (pending review)
  - If the user chose "I'll review it myself first": Status 鈫?"Designed"
...
- "What's next?"
  - Options:
    - "Design next system ([next-in-order])" 鈥?if undesigned systems remain
    - "Fix review findings" 鈥?if design-review flagged issues
    - "Stop here for this session"
    - "Run `/gate-check`" 鈥?if enough MVP systems are designed
...
- Agents do NOT write to files directly 鈥?the main session owns all file writes
...
1. Read `production/session-state/active.md` 鈥?it records the current system and
   which sections are complete
2. Read `design/gdd/[system-name].md` 鈥?sections with real content are done;
   sections with `[To be designed]` still need work
3. Resume from the next incomplete section 鈥?no need to re-discuss completed ones
...
7. **Specialist routing**: Complex sections get expert agent input, presented to
   the user for decision 鈥?never written silently
