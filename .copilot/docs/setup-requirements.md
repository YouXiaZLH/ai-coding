...
All hooks fail gracefully if tools are missing 鈥?nothing will break, but
...
| **GitHub Copilot** | AI agent CLI | `npm install -g <GitHub Copilot 官方安装命令>` |
...
- Hooks use `bash .copilot/hooks/[name].sh` 鈥?this works on Windows because
  GitHub Copilot invokes commands through a shell that can find `bash.exe`
...
GitHub Copilot works with any editor, but the template is optimized for:
- **VS Code** with the GitHub Copilot extension
- **Cursor** (GitHub Copilot compatible)
- Terminal-based GitHub Copilot CLI
