...
This is a complete GitHub Copilot agent architecture for game development. It
...
for Godot, Unity, and Unreal 鈥?each with dedicated sub-specialists for
...
|---------|-------------|
| `/start` | First-time onboarding 鈥?asks where you are, guides you to the right workflow |
...
Templates are in `.copilot/docs/templates/`:
...
1. **Run `/start`** (or `/brainstorm open`) 鈥?guided creative exploration:
...
2. **Set up the engine** 鈥?Run `/setup-engine` (uses the brainstorm recommendation)
   - Configures COPILOT.md, detects knowledge gaps, populates reference docs
   - Creates `.copilot/docs/technical-preferences.md` with naming conventions,
     performance budgets, and engine-specific defaults
   - If the engine version is newer than the LLM's training data, it fetches
     current docs from the web so agents suggest correct APIs
3. **Validate the concept** 鈥?Run `/design-review design/gdd/game-concept.md`
4. **Decompose into systems** 鈥?Run `/map-systems` to map all systems and dependencies
5. **Design each system** 鈥?Run `/design-system [system-name]` (or `/map-systems next`)
   to write GDDs in dependency order
6. **Test the core loop** 鈥?Run `/prototype [core-mechanic]`
7. **Playtest it** 鈥?Run `/playtest-report` to validate the hypothesis
8. **Plan the first sprint** 鈥?Run `/sprint-plan new`
9. Start building
...
1. **Set up the engine** 鈥?Run `/setup-engine [engine] [version]`
   (e.g., `/setup-engine godot 4.6`) 鈥?also creates technical preferences
2. **Write the Game Pillars** 鈥?delegate to `creative-director`
3. **Decompose into systems** 鈥?Run `/map-systems` to enumerate systems and dependencies
4. **Design each system** 鈥?Run `/design-system [system-name]` for GDDs in dependency order
5. **Create the initial ADR** 鈥?Run `/architecture-decision`
6. **Create the first milestone** in `production/milestones/`
7. **Plan the first sprint** 鈥?Run `/sprint-plan new`
8. Start building
...
1. **Run `/setup-engine`** with no arguments 鈥?it will ask about your game's
...
1. **Run `/start`** (or `/project-stage-detect`) 鈥?analyzes what exists,
   identifies gaps, and recommends next steps
2. **Configure engine if needed** 鈥?Run `/setup-engine` if not yet configured
3. **Validate phase readiness** 鈥?Run `/gate-check` to see where you stand
4. **Plan the next sprint** 鈥?Run `/sprint-plan new`

## File Structure Reference

```
COPILOT.md                          -- Master config (read this first, ~60 lines)
.copilot/
  settings.json                    -- GitHub Copilot hooks and project settings
